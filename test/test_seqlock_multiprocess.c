...
/* A writer killed between claiming the slot and turning the sequence odd leaves
 * an even sequence behind a dead owner, so the reclaim cannot key off odd. */
static int recovers_from(shrreg_proc_slot_t *slot, int leave_odd) {
    pid_t kid = fork();
    if (kid < 0) {
        perror("FAIL: fork");
        return 1;
    ...
    atomic_store(&slot->seq_owner, kid);
    if (leave_odd)
        atomic_fetch_add(&slot->seqlock, 1);

    seqlock_write_begin(slot);
    seqlock_write_end(slot);

    if (atomic_load(&slot->seqlock) & 1) {
        fprintf(stderr, "FAIL: slot left odd after reclaiming pid %d\n", kid);
        return 1;
    }
    printf("reclaimed a dead owner holding an %s sequence\n",
           leave_odd ? "odd" : "even");
    return 0;
}
...
int main(void) {
    ...
    if (recovers_from(region_info.my_slot, 0) != 0 ||
        recovers_from(region_info.my_slot, 1) != 0)
        return 1;
