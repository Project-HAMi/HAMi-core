...
unsigned int cuda_to_nvml_map(unsigned int dev) {
    EXPECT(dev == 0); return 3;
}
uint64_t get_current_device_memory_usage(const int dev) {
    EXPECT(dev == 3); return usage;
}
uint64_t get_current_device_memory_limit(const int dev) {
    EXPECT(dev == 0); return limit;
}
...
static void test_query(void) {
    ...
    for (int l = 0; l < 3; l++) for (int u = 0; u < 5; u++) {
        ...
        /* Unlimited overcommit arithmetic is outside this compatibility test. */
        if (limit == 0 && usage > 1000) continue;
