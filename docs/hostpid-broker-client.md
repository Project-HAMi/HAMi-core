...
`postInit()` selects the broker result, cache-local fallback, or node-wide fallback through one tested decision function. When the gate is enabled, a successful broker result selects the broker path, so neither fallback lock is selected. When the gate is disabled, the production client skips the broker request and selects the cache-local path. A failed broker result selects the node-wide path when the gate is enabled.
